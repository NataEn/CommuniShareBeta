...
export default function Items(props) {
    ...
    const item_view = (item,index) => (
        <div className="card col-3" key={index}>
            {item.images.length > 0 ? item.images.map(img => (
                    <img src={img} className="card-img-top" alt="Card image cap" key={img.name}/>)) :
    ...
    return (<div className='row'>
        {foundItems.length>0 ? foundItems.map((item,index)=> item_view(item,index)) : <div>no items were found</div>}
    </div>);
};